...
typedef struct primeResult PrimeResult;


PrimeResult isPrimeNaive(unsigned int number)
{
    PrimeResult result = {false, 0};
    ...
    // Instead of checking up to sqrt(number), we check up to number/2
    for (int i = 2; i <= number / 2; i++) {
        result.trials++;
        if (number % i == 0) {
            result.is_prime = false;
            result.trials = 0;           // optional: reset trials when composite
            return result;
        }
    }
...
void printPrimesNaiveUpTo(int limit)
{
    int total_trials = 0;

    printf("\nPrime Numbers between 2 and %d:\n", limit);
    printf("=== Naive method (checking up to n/2) ===\n");
    printf("2\n");  // the only even prime

    for (int i = 3; i <= limit; i += 2) {
        PrimeResult res = isPrimeNaive(i);
